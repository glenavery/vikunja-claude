...
#: On the board, in Ready, with a description made of real markup. Two numbers,
#: and they disagree: the board shows this task as **#8** while Vikunja's own
#: id for it is 9. The fixture also holds a *different* task at #9 (id 10), so
#: a call that confused the two would not fail — it would quietly edit the
#: wrong ticket, which is what these tests are here to catch.
TASK = 9
TASK_NUMBER = 8
...
#: A task that exists in Vikunja but on somebody else's board. It carries a
#: project-local number too — its own board's #1 — and that number reaching
#: this board's tools must resolve to nothing rather than to it.
FOREIGN = 777
FOREIGN_NUMBER = 1
...
class EditTestCase(McpTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.vikunja = FakeVikunja(
            ...
            foreign=[task(
                FOREIGN, "Someone else's ticket", "2026-07-01T00:00:00Z", index=1
            )],
        )
...
class TestThePreviewChangesNothing(EditTestCase):
    def test_a_call_without_a_token_returns_the_exact_current_and_proposed(self):
        previewed = self.preview(task_number=TASK_NUMBER, title=NEW_TITLE, description=NEW_TEXT)
    ...
    def test_the_current_value_is_the_text_a_human_reads_not_the_markup(self):
        ...
        previewed = self.preview(task_number=TASK_NUMBER, title=NEW_TITLE)
    ...
    def test_previewing_the_same_change_twice_issues_one_approval(self):
        first = self.preview(task_number=TASK_NUMBER, title=NEW_TITLE)
        second = self.preview(task_number=TASK_NUMBER, title=NEW_TITLE)
    ...
    def test_previewing_a_different_change_issues_a_different_approval(self):
        first = self.preview(task_number=TASK_NUMBER, title=NEW_TITLE)
        second = self.preview(task_number=TASK_NUMBER, title=NEW_TITLE + " again")
    ...
    def test_an_abandoned_preview_does_not_grow_without_bound(self):
        for index in range(MAX_PENDING_APPROVALS + 5):
            self.preview(task_number=TASK_NUMBER, title=f"#33 Title {index}")
...
class TestAnApprovedUpdateIsApplied(EditTestCase):
    def test_the_title_alone_can_be_replaced(self):
        result = self.approved_update(task_number=TASK_NUMBER, title=NEW_TITLE)
    ...
    def test_replacing_the_title_keeps_the_description(self):
        ...
        self.approved_update(task_number=TASK_NUMBER, title=NEW_TITLE)
        self.assertEqual(html_to_text(self.stored()["description"]), CURRENT_TEXT)

    def test_the_description_alone_can_be_replaced(self):
        result = self.approved_update(task_number=TASK_NUMBER, description=NEW_TEXT)
    ...
    def test_both_fields_can_be_replaced_in_one_write(self):
        result = self.approved_update(task_number=TASK_NUMBER, title=NEW_TITLE, description=NEW_TEXT)
    ...
    def test_the_description_is_escaped_never_interpreted(self):
        self.approved_update(task_number=TASK_NUMBER, description="Watch <script>alert(1)</script>")
...
class TestAnUpdateWithoutApprovalIsNotApplied(EditTestCase):
    def test_one_call_can_never_be_enough(self):
        self.preview(task_number=TASK_NUMBER, title=NEW_TITLE, description=NEW_TEXT)
        self.assertEqual(self.stored()["title"], CURRENT_TITLE)
        self.assertWroteNothing()
    ...
    def test_an_invented_token_is_refused(self):
        with self.assertRaises(ToolError) as caught:
            self.service.update_task(
                task_number=TASK_NUMBER, title=NEW_TITLE, approval_token="not-a-real-token"
            )
    ...
    def test_an_approval_cannot_be_spent_twice(self):
        ...
        self.service.update_task(task_number=TASK_NUMBER, title=NEW_TITLE, approval_token=token)
    ...
    def test_an_approval_for_one_task_cannot_be_used_on_another(self):
        ...
        with self.assertRaises(ToolError) as caught:
            self.service.update_task(
                task_number=9, title=NEW_TITLE, approval_token=previewed["approval_token"]
            )
        self.assertIn("Nothing was changed", str(caught.exception))
    ...
    def test_a_task_that_moved_under_the_approval_is_refused(self):
        ...
        previewed = self.preview(task_number=TASK_NUMBER, description=NEW_TEXT)
    ...
    def test_a_refused_approval_is_not_left_usable(self):
        ...
        token = previewed["approval_token"]
        with self.assertRaises(ToolError):
            self.service.update_task(task_number=TASK_NUMBER, title="#33 Other", approval_token=token)
...
class TestUpdateRefusals(EditTestCase):
    def test_a_task_on_another_board_is_refused(self):
        """Its board numbers it #1; this board has no #1, and that is the end of it."""
        with self.assertRaises(ToolError) as caught:
            self.service.update_task(
                task_number=FOREIGN_NUMBER, title="Renamed by the connector"
            )
        message = str(caught.exception)
    ...
    def test_a_number_no_task_on_the_board_carries_is_refused(self):
        with self.assertRaises(ToolError) as caught:
            self.service.update_task(task_number=4242, title="Renamed")
        self.assertIn("4242", str(caught.exception))
        self.assertWroteNothing()

    def test_naming_no_field_at_all_is_refused(self):
        with self.assertRaises(ToolError) as caught:
            self.service.update_task(task_number=TASK_NUMBER)
        self.assertIn("complete replacement title", str(caught.exception))
        self.assertWroteNothing()

    def test_a_blank_title_is_refused(self):
        with self.assertRaises(ToolError):
            self.service.update_task(task_number=TASK_NUMBER, title="   ")
        self.assertEqual(self.stored()["title"], CURRENT_TITLE)

    def test_a_blank_description_is_refused(self):
        with self.assertRaises(ToolError):
            self.service.update_task(task_number=TASK_NUMBER, description="  \n ")
        self.assertEqual(html_to_text(self.stored()["description"]), CURRENT_TEXT)

    def test_an_oversized_title_is_refused(self):
        with self.assertRaises(ToolError):
            self.service.update_task(task_number=TASK_NUMBER, title="x" * (MAX_TITLE_CHARS + 1))
        self.assertWroteNothing()

    def test_an_oversized_description_is_refused(self):
        with self.assertRaises(ToolError):
            self.service.update_task(
                task_number=TASK_NUMBER, description="x" * (MAX_DESCRIPTION_CHARS + 1)
            )
        self.assertWroteNothing()
...
class TestUpdateIsIdempotent(EditTestCase):
    def test_asking_for_the_value_the_task_already_holds_is_a_no_op(self):
        result = self.service.update_task(task_number=TASK_NUMBER, title=CURRENT_TITLE)
    ...
    def test_repeating_an_applied_change_writes_nothing_a_second_time(self):
        ...
        repeated = self.service.update_task(
            task_number=TASK_NUMBER, title=NEW_TITLE, description=NEW_TEXT
        )
    ...
    def test_one_of_two_fields_already_matching_narrows_the_change(self):
        result = self.preview(task_number=TASK_NUMBER, title=CURRENT_TITLE, description=NEW_TEXT)
        self.assertEqual(result["changed_fields"], ["description"])

    def test_whitespace_around_the_same_value_is_still_a_no_op(self):
        result = self.service.update_task(task_number=TASK_NUMBER, title=f"  {CURRENT_TITLE}  ")
        self.assertEqual(result["changed_fields"], [])
...
class TestComments(EditTestCase):
    ...
    def test_the_preview_writes_nothing_and_names_the_target(self):
        previewed = self.service.add_task_comment(task_number=TASK_NUMBER, comment=self.TEXT)
    ...
    def test_the_comment_comes_back_through_get_task(self):
        self.approved_comment(task_number=TASK_NUMBER, comment=self.TEXT)
        read = self.service.get_task(TASK_NUMBER)
        self.assertEqual([c["text"] for c in read["comments"]], [self.TEXT])

    def test_the_comment_is_escaped_never_interpreted(self):
        self.approved_comment(task_number=TASK_NUMBER, comment="Careful: <script>alert(1)</script>")
    ...
    def test_a_comment_without_approval_is_not_written(self):
        self.service.add_task_comment(task_number=TASK_NUMBER, comment=self.TEXT)
        self.assertEqual(self.comments_on(), [])

    def test_an_invented_token_is_refused(self):
        with self.assertRaises(ToolError) as caught:
            self.service.add_task_comment(
                task_number=TASK_NUMBER, comment=self.TEXT, approval_token="nope"
            )
        self.assertIn("Nothing was changed", str(caught.exception))
        self.assertEqual(self.comments_on(), [])
    ...
    def test_an_update_approval_cannot_be_redeemed_as_a_comment(self):
        ...
        previewed = self.service.update_task(task_number=TASK_NUMBER, title=NEW_TITLE)
        with self.assertRaises(ToolError):
            self.service.add_task_comment(
                task_number=TASK_NUMBER,
                comment=self.TEXT,
    ...
    def test_the_same_comment_twice_is_written_once(self):
        first = self.approved_comment(task_number=TASK_NUMBER, comment=self.TEXT)
        ...
        second = self.service.add_task_comment(task_number=TASK_NUMBER, comment=self.TEXT)
    ...
    def test_a_duplicate_is_recognised_across_a_restart(self):
        ...
        result = restarted.add_task_comment(task_number=TASK_NUMBER, comment=self.TEXT)
    ...
    def test_a_duplicate_of_somebody_elses_comment_is_also_suppressed(self):
        ...
        result = self.service.add_task_comment(task_number=TASK_NUMBER, comment=self.TEXT)
    ...
    def test_a_genuinely_different_comment_is_written(self):
        self.approved_comment(task_number=TASK_NUMBER, comment=self.TEXT)
        second = self.approved_comment(task_number=TASK_NUMBER, comment="A second, different note.")
    ...
    def test_a_task_on_another_board_is_refused(self):
        with self.assertRaises(ToolError) as caught:
            self.service.add_task_comment(task_number=FOREIGN_NUMBER, comment=self.TEXT)
    ...
    def test_a_number_no_task_on_the_board_carries_is_refused(self):
        with self.assertRaises(ToolError) as caught:
            self.service.add_task_comment(task_number=4242, comment=self.TEXT)
        self.assertIn("4242", str(caught.exception))
        self.assertWroteNothing()

    def test_a_blank_comment_is_refused(self):
        with self.assertRaises(ToolError):
            self.service.add_task_comment(task_number=TASK_NUMBER, comment="   ")
        self.assertWroteNothing()

    def test_an_oversized_comment_is_refused(self):
        with self.assertRaises(ToolError):
            self.service.add_task_comment(
                task_number=TASK_NUMBER, comment="x" * (MAX_COMMENT_CHARS + 1)
            )
        self.assertWroteNothing()
...
class TestThroughTheProtocol(EditTestCase):
    ...
    def test_an_update_takes_two_calls_over_the_protocol(self):
        previewed = self.result("update_task", task_number=TASK_NUMBER, title=NEW_TITLE)[
            "structuredContent"
        ]
        self.assertFalse(previewed["applied"])
        self.assertEqual(self.stored()["title"], CURRENT_TITLE)
    ...
    def test_a_comment_takes_two_calls_over_the_protocol(self):
        previewed = self.result("add_task_comment", task_number=TASK_NUMBER, comment="Noted.")[
            "structuredContent"
        ]
        self.assertFalse(previewed["added"])
    ...
    def test_a_refusal_is_a_visible_error_not_a_silent_success(self):
        result = self.result("update_task", task_number=FOREIGN_NUMBER, title="Renamed")
        self.assertTrue(result["isError"])
        self.assertIn("Nothing was changed", result["content"][0]["text"])

    def test_update_task_requires_a_task_number_before_the_tool_runs(self):
        response = self.protocol.handle(
            {
                "jsonrpc": "2.0",
                "id": 1,
